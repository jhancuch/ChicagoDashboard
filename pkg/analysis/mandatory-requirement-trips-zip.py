"""
forecast the daily, weekly, and monthly taxi trips for every zip code
"""
...
def forecast_zip(df, x, z, col):
    ...
    if len(dat) == 365:
        ...
        #dat = dat.set_index('trip_start_timestamp').resample('D').ffill().reset_index()
        dat = dat.asfreq('d')
        print(dat)

    train_exogenous = df[df['zip'] == z]
    ...
    if len(train_exogenous) == 365:
        ...
        idx = pd.date_range('2020-10-01', '2021-09-30')
        train_exogenous.sort_values('trip_start_timestamp', ascending=True, inplace=True)
        #train_exogenous = train_exogenous.set_index('trip_start_timestamp').resample('D').ffill().reset_index()
        train_exogenous.set_index('trip_start_timestamp', inplace=True)
...
df_zip = pd.read_sql(zip_query, connection)
df_zip['count'] = df_zip['count'].astype(float).astype(int)


# Modify table so we have a count of trips to and from each zip code on each day
df_zip_pickup = df_zip[["trip_start_timestamp", "pickup_zip", "count"]].copy()
df_zip_dropoff = df_zip[["trip_start_timestamp", "dropoff_zip", "count"]].copy()

df_zip_pickup_final = df_zip_pickup[["trip_start_timestamp", "pickup_zip", "count"]].rename(columns = {"count": "pickup_count", "pickup_zip": "zip"})
df_zip_dropoff_final = df_zip_dropoff[["trip_start_timestamp", "dropoff_zip", "count"]].rename(columns = {"count": "dropoff_count", "dropoff_zip": "zip"})

temp = pd.merge(df_zip_pickup_final, df_zip_dropoff_final, how='inner')

aggregate = temp.groupby(["trip_start_timestamp", "zip"])[['pickup_count', 'dropoff_count']].sum().reset_index()
...
zips = list(aggregate["zip"].drop_duplicates())
...
for i in zips:
    print(i)
    temp = forecast_zip(aggregate, future, i, 'pickup_count')
    temp['zip'] = i
    final_pickup = final_pickup.append(temp)

for i in zips:
    print(i)
    temp = forecast_zip(aggregate, future, i, 'dropoff_count')
    temp['zip'] = i
...
final = pd.merge(final_pickup, final_dropoff, on = ['date', 'zip'])
...
# Day Charts
day_table = final_day[["date", "zip", "pickup_count", "dropoff_count"]].reset_index(drop=True).sort_values("zip")

day1 = plt.figure(figsize=(14,7))

ax = plt.gca()
plt.xlabel('ZIP Codes', figure=day1)
plt.ylabel('Forecasted Number of Trips', figure=day1)
plt.title('Forecasted Number of Trips to and from ZIP Codes on {0}'.format(min(final_day["date"])), figure=day1)
x = list(range(len(day_table["zip"])))
labels = day_table["zip"]
...
plt.legend(["Forecasted Trips From ZIP Code", "Forecasted Trips To ZIP Code"])

# Week Charts
week_table = final_week[["date", "zip", "pickup_count", "dropoff_count"]].reset_index(drop=True).sort_values("zip")

# Group by ZIP
chart_by_zip_week = week_table.groupby(["zip"])["pickup_count", "dropoff_count"].sum().reset_index()
chart_by_zip_week.loc[:, "Total Trips"] = chart_by_zip_week["pickup_count"] + chart_by_zip_week["dropoff_count"]
chart_by_zip_week = chart_by_zip_week.sort_values("zip")

# Charts
week1 = plt.figure(figsize=(14,7))

ax = plt.gca()
plt.xlabel('ZIP Codes', figure=week1)
plt.ylabel('Forecasted Number of Trips', figure=week1)
plt.title('Forecasted Total Number of Trips to and from ZIP Codes from {0} to {1}'.format(min(final_week["date"]), max(final_week["date"])), figure=week1)
x = list(range(len(chart_by_zip_week["zip"])))
labels = chart_by_zip_week["zip"]

plt.bar(x, chart_by_zip_week["pickup_count"], label = 'Pickups', color='#9ecae1', figure=week1)
plt.bar(x, chart_by_zip_week["dropoff_count"], label = 'Dropoffs', bottom = chart_by_zip_week["pickup_count"], color = '#08306b', figure=week1)
plt.ticklabel_format(style='plain')
plt.xticks(x, labels, rotation=90)
...
plt.title('Forecasted Total Number of Trips to and from ZIP Codes from {0} to {1}'.format(min(final_week["date"]), max(final_week["date"])), figure=week2)
...
plt.legend(["Trips From ZIP Code", "Trips To ZIP Code"])


# Month Charts
month_table = final_month[["date", "zip", "pickup_count", "dropoff_count"]].reset_index(drop=True).sort_values("zip")

# Group by ZIP
chart_by_zip_month = month_table.groupby(["zip"])["pickup_count", "dropoff_count"].sum().reset_index()
chart_by_zip_month.loc[:, "Total Trips"] = chart_by_zip_month["pickup_count"] + chart_by_zip_month["dropoff_count"]
chart_by_zip_month = chart_by_zip_month.sort_values("zip")

# Charts
month1 = plt.figure(figsize=(14,7))

ax = plt.gca()
plt.xlabel('ZIP Codes', figure=month1)
plt.ylabel('Forecasted Number of Trips', figure=month1)
plt.title('Forecasted Total Number of Trips to and from ZIP Codes from {0} to {1}'.format(min(final_month["date"]), max(final_month["date"])), figure=month1)
x = list(range(len(chart_by_zip_month["zip"])))
labels = chart_by_zip_month["zip"]

plt.bar(x, chart_by_zip_month["pickup_count"], label = 'Pickups', color='#9ecae1', figure=month1)
plt.bar(x, chart_by_zip_month["dropoff_count"], label = 'Dropoffs', bottom = chart_by_zip_month["pickup_count"], color = '#08306b', figure=month1)
plt.ticklabel_format(style='plain')
plt.xticks(x, labels, rotation=90)
...
plt.title('Forecasted Total Number of Trips to and from ZIP Codes from {0} to {1}'.format(min(final_month["date"]), max(final_month["date"])), figure=month2)
#ax.set_xticklabels(labels=chart_by_date_month["date"], rotation=0, figure=month2)
...
save(day1, "day1-zip.png")
save(week1, "week1-zip.png")
save(week2, "week2-zip.png")
save(month1, "month1-zip.png")
save(month2, "month2-zip.png")


# Save data in Postgresql
insert = final[["date", "zip", "pickup_count", "dropoff_count"]]

# Save tables in postgresql
cursor.execute("DELETE FROM trips_zip_prediction")
connection.commit()
...
query  = "INSERT INTO trips_zip_prediction({0}) VALUES ".format(cols) + ",".join(values)
